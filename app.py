...
with st.sidebar:
    ...
    if st.button('Actualizar fuente y caché', width='stretch'):
        try:
            ...
            st.session_state["source_warnings"] = svc.warnings
            st.cache_data.clear()
            st.rerun()
...
metadata, domain_errors = {'QCL':qmeta}, {}
frames = LazyFrames(base, svc, scope, items, yr, metadata, domain_errors)
...
price = None
if len(scope) == 1 and len(items) == 1:
    try:
        price_meta = svc.cached_metadata('PP')
        price_rows = query('PP',price_meta['file'],scope,items,('Producer Price (USD/tonne)',),(latest,latest),svc).dropna(subset=['Value'])
        price = float(price_rows.Value.iloc[0]) if len(price_rows) == 1 else None
    except Exception as exc:
        domain_errors['PP'] = str(exc)
...
for message in dict.fromkeys(svc.warnings + st.session_state.pop("source_warnings", [])):
    st.warning(message)

# Preserve the state of existing inputs while their tab is not executed.
for key in ('data_domain', 'data_search', 'ai_question'):
    if key in st.session_state:
        st.session_state[key] = st.session_state[key]
tabs = st.tabs(['📊 Panorama','⚖️ Comparadores','🌍 Comercio y precios','📋 Datos','🤖 AgroData AI'], key='active_tab', on_change='rerun')
if tabs[0].open:
    with tabs[0]:
...
if tabs[3].open:
    with tabs[3]:
        metadata.update({code: svc.cached_metadata(code) for code in ('PP','TCL')})
