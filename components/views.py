...
def downloads(frames, metadata, years, scope, items):
    ...
    code = st.selectbox('Dominio de la tabla y CSV', list(frames), format_func=lambda c:{'QCL':'Producción / área / rendimiento','PP':'Precios al productor','TCL':'Comercio'}[c])
    data = frames[code]
    search = st.text_input('Buscar en la tabla (texto literal)')
...
def assistant(base, indicator, label, year):
    ...
    question = st.text_area('Pregunta sobre los datos cargados', placeholder='¿Cuál fue el mayor productor?')
